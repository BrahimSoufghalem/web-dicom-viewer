...
import { MeasurementsSidebarWrapper } from './features/dicom-viewer/components/MeasurementsSidebar';
import { LogOut, Loader, LayoutGrid, LayoutTemplate, Square, Columns, Globe, PanelLeft, PanelRight } from 'lucide-react';
import { useEffect, useState } from 'react';
import { initCornerstone } from './features/dicom-viewer/utils/initCornerstone';
import { useMeasurements } from './features/dicom-viewer/hooks/useMeasurements';
    ...
    <div className="app-container">
      {patients.length === 0 ? (
        ...
        <>
          ...
          <div className="toolbar-container" style={{ borderBottom: '1px solid var(--border-color)' }}>
            <Toolbar />
            ...
            <div className="tool-group">
              <button
                ...
                style={layout === '1x1' ? { color: '#a89fea', background: 'rgba(127,119,221,0.15)' } : {}}
              >
                <Square size={20} />
              ...
              <button
                ...
                style={layout === '1x2' ? { color: '#a89fea', background: 'rgba(127,119,221,0.15)' } : {}}
              >
                <Columns size={20} />
              ...
              <button
                ...
                style={layout === '1+2' ? { color: '#a89fea', background: 'rgba(127,119,221,0.15)' } : {}}
              >
              ...
              <button
                ...
                style={layout === '2x2' ? { color: '#a89fea', background: 'rgba(127,119,221,0.15)' } : {}}
              >
                <LayoutGrid size={20} />
            ...
            <div style={{ marginLeft: 'auto', display: 'flex', gap: '8px' }}>
              <div className="tool-group">
                <button
                  className={`tool-btn ${isMeasurementsOpen ? 'active' : ''}`}
                  onClick={() => setIsMeasurementsOpen(!isMeasurementsOpen)}
                  style={isMeasurementsOpen ? { color: '#3ecf9b', background: 'rgba(29,158,117,0.15)' } : {}}
                >
                  <PanelRight size={20} />
                  <span className="tooltip">{t('measurements.title')}</span>
                </button>
              </div>
              ...
              <div className="tool-group" style={{ position: 'relative' }}>
                ...
                {showLangMenu && (
                  <div className="preset-menu" style={{ position: 'absolute', top: 'calc(100% + 4px)', left: '50%', transform: 'translateX(-50%)', background: 'var(--bg-panel)', padding: '0.25rem', borderRadius: '4px', zIndex: 9999, display: 'flex', flexDirection: 'column', minWidth: '100px', boxShadow: '0 4px 6px rgba(0,0,0,0.3)', border: '1px solid var(--border)' }}>
          ...
          <div style={{ display: 'flex', flex: 1, position: 'relative', overflow: 'hidden', minHeight: 0 }}>
            ...
            <div className={`left-sidebar-wrapper ${isSidebarOpen ? 'open' : 'closed'}`}>
              <SeriesSidebar />
            </div>
            ...
            <div className={`right-sidebar-wrapper ${isMeasurementsOpen ? 'open' : 'closed'}`}>
              <MeasurementsSidebarWrapper 
                measurements={measurements} 
                removeMeasurement={removeMeasurement} 
                jumpToMeasurement={jumpToMeasurement} 
                updateMeasurementLabel={updateMeasurementLabel}
              />
            </div>
          </div>
        </div>
